% Test MSE by model and encoding for one dataset of the nested-LOSO run.
% The caller sets \loMsePlotData to data/paper_loso_<dataset>_mse_plot.csv
% (wide: encoding, then one MSE column per model: mean, pca, snn_ae, lstm_ae,
% gru_ae, transformer_ae). The mean-frame column is omitted here for scale.
\pgfplotstableread[col sep=comma]{\loMsePlotData}\msetable
