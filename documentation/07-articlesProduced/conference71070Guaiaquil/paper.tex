...
Metrics include MSE, MAE, $R^2$, F1, spike rate, estimated energy, timing,
parameters, and MACs.
...
Computational energy is estimated from operation counts.
SNN energy is proportional to spike rate $r$ and MAC count $M$; LSTM
energy depends on $M$ alone. All estimates use relative units (no absolute
hardware assumptions). Hardware energy per operation varies by platform
and will be measured in future work.
