...
comparable.
Table~\ref{tab:recon} shows the per-encoding breakdown: latency encoding
consistently achieves lower MSE than direct encoding across all SNN modes
(e.g., SNN-conv1d/latency MSE $= 0.103$, $R^2=0.527$), while direct encoding
is most challenging for all models.
...
Encoding scheme is the primary factor for reconstruction fidelity: latency
encoding consistently yields lower MSE and higher $R^2$ than direct encoding
across all SNN modes, because the binary time-to-first-spike signal retains
relative amplitude ordering without the high-variance density of raw waveforms.
Poisson encoding is intermediate.
