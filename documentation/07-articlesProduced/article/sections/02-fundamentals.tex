\section{Biological and Computational Background}
	...
	\par In order to get the aforementioned advantages the Spiking Neural Networks (SNNs) can utilize \textbf{spikes} at the input, hidden and outputs layers. SNNs may have continuous inputs as well and keep its properties.
	
	\par SNNs \textbf{are not} a one-to-one simulation of neurons. Instead, it approximates certain computational capabilities of specific biological properties. Some studies like \cite{jones2020single} created models way closer to natural neurons exploring the nonlinearity of dendrites and another neuron features yielding remarkable results in the classification.
	
	\par As can be seen in Figure \ref{fig:neuronspike} SNNs neurons, given the correct parameters, are very noise tolerant because it acts as a \textbf{lowpass filter}. Generating spikes even when a considerably level of interference is present. The network neurons are very time-oriented too, being great when it comes to process streams of data \cite{10242251}.
