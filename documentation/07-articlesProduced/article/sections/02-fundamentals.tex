...
\label{sec:fundamentals}
	...
	\par Spiking Neural Networks (SNNs) aim to emulate some of these advantages by processing \textbf{spikes} at the input, hidden, and output layers. While SNNs can also accept continuous-valued inputs, they retain their event-driven properties and energy efficiency due to sparse activation patterns.
	...
	\subsection{Leaky Integrate and Fire Neuron}
		...
		\par Although the focus of this work is the \textit{Leaky Integrate-and-Fire} (LIF) neuron—due to its simplicity, computational efficiency, and its strong generalization performance across many tasks \cite{dan_goodman_2022_7044500}—there exist more biologically accurate models, such as the \textbf{Hodgkin–Huxley neuron} \cite{gerstner2014neuronal}, and others like the one proposed in \cite{jones2020single}, which incorporate dendritic nonlinearities and additional biophysical properties to more closely replicate real neuronal behavior.
		...
		\par LIF behaves much like Resistor-Capacitor circuit as illustrated in \autoref{fig:rcmodel}. In this analogy $R$ represents the leakage resistance, $I_{in}$ the input current, $C$ the membrane capacitance, $U_{mem}$ the membrane potential (i.e., the accumulated charge) and $v$ a switch that lets the capacitor discharge (i.e. emit a spike) when a voltage threshold is reached.
		...
		\par As a result of the aforementioned characteristics, information in SNNs is encoded in terms of the \textit{timing} and/or \textit{rate} of spikes. This enables effective processing of temporal data streams but imposes limitations when dealing with static data.
		
	\subsection{Mathematical Model of the LIF Neuron}
	
		\par LIF model is governed by the Equations \cite{10242251} bellow:
		...
		\par To know how these charge changes according to the time (aka current) we can derivate $Q$ as in \autoref{eq:rateOfChargeChange}. This expression express the current in the capacitive part of the neuron $I_C$
		...
		\par From that, and setting $I_{in} = 0$ (i.e. no input) and considering $\tau = R.C$ is a constant and a starting voltage $V_{mem}(0)$ the neuron's voltage behavior can be modeled as an exponential curve as can be seen in \autoref{eq:expmembrane}.
 		...
 		\par Then one can say that: In the absence of an input $I_{in}$, the membrane potential decays exponentially as illustrated in \autoref{fig:membranepotentialdecay} and implemented in  \autoref{lst:membranepotentialdecay}.
			...
			\par Note that when action potentials increases there is still an exponential behavior as seen in \autoref{fig:membranepotentialincrease} and implemented in \autoref{lst:membranepotentialincrease}.
			
				\input{listings/membranepotentialincrease.cpp}
 	...
 	\subsection{Training}
		...
		\par A SNN neuron has an activation-function behavior that is more relatable to a \textbf{step function}. Therefore, in principle, we can't use gradient descent-based solutions because this kind of function \textbf{is not} differentiable \cite{kasabov2019time}.
		
		\par But there are some insights out there that may shed some light on this subject: While some \textit{in vivo/in vitro} observations show that brains, in general, learn by strengthening/weakening and adding/removing synapses or even by creating new neurons or other cumbersome methods like RNA packets, there are some more acceptable methods like the ones listed bellow \cite{kasabov2019time}:
