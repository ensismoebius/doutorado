...
\label{sec:fundamentals}
	...
	\subsection{Leaky Integrate and Fire Neuron}
		...
		\par LIF behaves much like Resistor-Capacitor circuit as illustrated in Figure \ref{fig:rcmodel}. In this analogy $R$ represents the leakage resistance, $I_{in}$ the input current, $C$ the membrane capacitance, $U_{mem}$ the membrane potential (i.e., the accumulated charge) and $v$ a switch that lets the capacitor discharge (i.e. emit a spike) when a voltage threshold is reached.
		...
		\par Spikes are represented as \textbf{sparsely} distributed \textbf{ones} in a train of \textbf{zeros}, as illustrated in Figure \ref{fig:spikessparsitystaticsupress} and \ref{fig:sparsity}. This approach simplify the models and reduces the computational power and needed storage to run a SNN.\newline
	...
	\subsection{Mathematical Model of the LIF Neuron}
		...
		\par Let's consider $Q$ as a measurement of electrical charge and $V_{mem}(t)$ is the membrane potential in a certain time $t$ than the neuron capacitance $C$ is given by the Equation \ref{eq:capacitance}.
		...
		\par Therefore the neuron charge can be expressed as Equation \autoref{eq:charge}.
