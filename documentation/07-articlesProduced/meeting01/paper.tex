...
leave-one-group-out cross-validation} on three datasets --- spoken digits
(FSDD \cite{jackson2018fsdd} and AudioMNIST) and single-lead ECG (MIT-BIH
Arrhythmia) --- where the group is the speaker or the ECG record: no source
recording or group crosses a split, and SNN hyperparameters are selected inside
each fold on the held-out validation group only, then retrained on the combined
training and validation data before a single evaluation on the untouched test
group. Model differences are assessed per dataset with a recording-level paired
analysis (bootstrap confidence intervals over recordings, Wilcoxon signed-rank,
Holm--Bonferroni), with group-level and seed-level results reported as
generalization- and optimization-robustness evidence. We report reconstruction quality
...
Speaker (or patient) identity is the natural unit of generalization, and the
publicly available spoken-digit and ECG benchmarks used here have on the order of
tens of such groups, which also bounds how strong any population-level claim can
be. We therefore evaluate on three datasets and make the recording-level paired
difference, not the group-level one, the primary estimand.

\par This paper addresses these gaps with four contributions:
\begin{enumerate}
  \item A leakage-controlled protocol: \emph{nested six-fold
        leave-one-group-out cross-validation} on three datasets (FSDD
        \cite{jackson2018fsdd}, AudioMNIST, MIT-BIH Arrhythmia ECG) in which no
        source recording or group (speaker / record) crosses a split and SNN
...
\cite{cawley2010overfitting}. The benchmarks here have tens of groups at most, so
we treat the group-level result as a robustness check and make the
...
inner validation group. The configuration with the lowest validation MSE is
selected, then \emph{retrained} on the training and validation groups combined
and evaluated \emph{once} on the held-out test group. Because train and
validation groups share no recordings with the test group but not with each
...
summarized per dataset in the selection tables (e.g.\ Table~\ref{tab:snnsel-fsdd}).

\par \textbf{Fixed-architecture baselines.} LSTM-AE, GRU-AE, and Transformer-AE
use fixed profile architectures; their only use of the validation group is
early stopping, which never touches the test group. PCA and the mean-frame
...
\textbf{Datasets.}
\par We evaluate on three datasets, each a corpus of 1-D signals with a natural
group unit for leave-one-group-out (Table~\ref{tab:datasets}):
\begin{itemize}
  ...
  \item \textbf{MIT-BIH Arrhythmia} --- single-lead ECG (lead 0), 48 half-hour
        records at $360\,$Hz, converted to physical units from the WFDB header;
        group $=$ record. A few records share a subject (noted in
        Sec.~\ref{sec:discussion}).
\end{itemize}
Every recording is partitioned into non-overlapping windows of $T=256$ samples
with stride $S=T$. For FSDD all windows are used; for the two larger datasets a
per-recording cap keeps long or silence-padded recordings from dominating the
pooled window counts (2 windows/recording for AudioMNIST, 40 for MIT-BIH), which
also keeps the three datasets' training sizes comparable. Adjacent windows of
one recording remain temporally correlated, which motivates the recording-level
statistical unit (Sec.~\ref{sec:stats}).
...
\begin{tabular}{lllrr}
\toprule
Dataset & Signal & $G$ & \#$G$ / $K$ & cap \\
\midrule
FSDD       & digits, 8\,kHz     & speaker & 6 / 6  & --- \\
AudioMNIST & digits, 8\,kHz     & speaker & 60 / 6 & 2 \\
MIT-BIH    & ECG, 360\,Hz       & record  & 48 / 6 & 40 \\
\bottomrule
\end{tabular}
\label{tab:datasets}
\end{table}
...
MIT-BIH: 8 records per block) it is grouped leave-one-block-out. Every window is
...
test split, aggregated across the six folds. SNN hyperparameters are selected on
the validation block only, then retrained on train $\cup$ validation before a
single evaluation on the test block (Sec.~\ref{sec:baselines}).
...
the three datasets, six outer folds, five seeds ($42$--$46$), the four trained
families plus the two linear references, three encodings, and the SNN selection
grid ($3\times3\times3$). The run script iterates over (dataset, fold); a
profile audit test verifies the profile parses and validates.
...
\par \textbf{Robustness --- group level.} Per held-out group (speaker or ECG
record) we form the mean error and run a paired Wilcoxon / sign test. This checks
whether the sign and rough magnitude of a difference hold across a dataset's
groups; with only tens of groups it is not strong population-level inference and
is reported as such.
...
recording-level interval excludes zero. Because SNN selection is redone inside
every fold, ``SNN-AE'' denotes a \emph{selection procedure}; disagreement of the
selected mode across folds is itself a finding about how encoding-dependent the
best pre-processing is. The cost columns ($P$, $C_{\mathrm{raw}}$, timing) are
architecture properties and do not change with the dataset; the Transformer's
$O(T^2 d_{\mathrm{model}})$ self-attention term shows up in its $C_{\mathrm{raw}}$
independently of its MSE.
...
\resultsForDataset{mitbih}{MIT-BIH Arrhythmia (single-lead ECG, 48 records)}

\par \emph{Reading across datasets.} A model family's advantage is credible to
the extent it recurs on all three datasets' recording-level analyses; an effect
...
task at two speaker-pool sizes; MIT-BIH tests whether the ranking transfers to a
different 1-D signal.
...
\emph{Limitations.}
\begin{itemize}
  \item \textbf{Few groups.} The three datasets have 6, 60, and 48 groups; the
        group-level analysis is a robustness check, not population inference,
        which is why the recording-level analysis is the primary estimand.
  \item \textbf{Three small datasets.} Two are the same task (spoken digits) and
        one is ECG; the ranking may not transfer to other 1-D signal domains.
        AudioMNIST and MIT-BIH are additionally window-capped per recording (2
        and 40) to bound compute, and a few MIT-BIH records share a subject, so
        its record-level grouping is a slight over-count of independent subjects.
  ...
  \item \textbf{Dimension-matched, not parameter-matched.} SNN-AE, LSTM-AE,
        GRU-AE, and Transformer-AE share the same hidden width ($H=64$) and
        latent size ($d=32$), not the same trainable parameter count $P$.
        Dropping the LSTM cell state removes one gate's worth of parameters
        per layer, so GRU-AE is strictly smaller than LSTM-AE at identical
        $H$; the Transformer-AE is sized \emph{near}, not equal to, the
        recurrent baselines; and the SNN-AE carries its own biophysical
        parameters ($R$, $C$, $V_{th}$) with no non-spiking analogue. This
        follows the common convention in SNN-vs-ANN comparisons of matching
        architectural dimensions rather than raw parameter count, but it
        means a quality gap partly reflects a capacity gap, not only an
        inductive-bias difference. The summary tables report each model's
        real $P$ (Table~\ref{tab:perf-fsdd} and following) precisely so cost
        and quality can be read together rather than assumed equal.
\end{itemize}
...
reconstruction on three datasets (FSDD, AudioMNIST, MIT-BIH ECG). The evaluation
is leakage-controlled: nested six-fold
leave-one-group-out cross-validation with no recording or group crossing a
split and SNN hyperparameters selected on the inner validation group only.
