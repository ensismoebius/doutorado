	...
	\section{Estrutura da estratégia proposta}
		...
		\subsection{Protocolo experimental em duas fases}
			...
			\subsubsection{Fase 00 --- construção e seleção do vetor de características}
				...
				\par A Fase 00 é aplicada separadamente à voz e ao EEG. Para o método manual, varrem-se duas escolhas: a \textbf{wavelet-mãe} da DTWPT (23 opções: \texttt{haar} e \texttt{daub4} a \texttt{daub46}) e a \textbf{escala} de agrupamento espectral (\textit{bark}, \textit{mel}, \textit{lfcc}). Além disso, escolhe-se a \textbf{categoria} do descritor: Categoria~1 usa as energias de banda diretamente; Categoria~2 aplica logaritmo e DCT sobre essas energias, gerando os coeficientes cepstrais LFCC, MFCC ou BFCC (para escala \textit{lfcc}, \textit{mel} ou \textit{bark}). Isso dá $23 \times 3 \times 2 = 138$ combinações manuais por sinal, às quais se somam seis \textit{autoencoders} compactos, sem wavelet ou escala: duas famílias --- \textbf{SNN-AE} (autoencoder pulsado, codificador \texttt{Linear}$\to$\texttt{LIF} e decodificador \texttt{Linear}$\to$\texttt{LIF-integrador}) e \textbf{ANN-AE} (autoencoder denso não-pulsado) --- em três tamanhos cada (\texttt{tiny}, \texttt{small}, \texttt{base}). Ambas as famílias consomem um vetor achatado de dimensão fixa (o sinal é reamostrado por \textit{pooling} para $256$ amostras), usam uma única camada oculta (rasa, adequada a dispositivos de baixo poder) e razão de compressão $2\!:\!1$ entre a camada oculta e o gargalo latente (dimensões latentes $8$, $16$ e $32$). O latente é o próprio vetor de características, então um gargalo menor gera vetor menor, barateando o classificador a jusante --- princípio da literatura de autoencoders leves para borda. Cada combinação gera um vetor de características, avaliado pela distância $D_{\text{verdade}}$ --- a distância ao vértice \textit{Verdade} no plano paraconsistente (\autoref{sec:conceitos}); quanto menor, mais separáveis são as classes antes de qualquer classificador. O vencedor do sinal é a combinação de menor $D_{\text{verdade}}$. Nenhum classificador é treinado nesta fase. O \autoref{lst:e05Phase00} descreve o procedimento e a \autoref{fig:e05Phase00Flow} o resume.
				...
				\par Considerando os dois sinais, a Fase 00 tem $2 \times (138 + 6) = 288$ execuções de ranking. O vetor fundido não é varrido aqui: ele é montado na Fase 01, a partir dos vencedores de cada sinal.
