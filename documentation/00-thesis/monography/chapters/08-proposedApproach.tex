\chapter{Abordagem proposta}
	...
	\section{Estrutura da estratégia proposta}
		...
		\par Quanto à extração automatizada, serão comparados dois tipos de \textit{autoencoder} --- um de pulsos (\ac{SNN}-AE) e um convencional (\ac{ANN}-AE) --- e ambos serão confrontados com a extração manual (\textit{handcrafted}), usando a engenharia paraconsistente de características como critério de comparação (\autoref{sec:conceitos}).
...
As medições sobre a grade completa da Fase~00 (18 perfis SNN-AE --- 3 codificações $\times$ 3 tamanhos $\times$ 2 sinais, 3 repetições cada) \textbf{contrariam a ordem inicialmente esperada}. Como o vencedor é, por definição, a combinação de \emph{menor} $D_{\text{verdade}}$, a ordem observada é \texttt{direct} $\succ$ \texttt{latency} $\succ$ \texttt{poisson}. Em \texttt{tiny}/EEG: \texttt{direct} ($\alpha=0{,}667$, $D_{\text{verdade}}=1{,}493$) supera \texttt{latency} ($\alpha=0{,}116$, $D_{\text{verdade}}=1{,}875$), que por sua vez supera \texttt{poisson} ($\alpha=0{,}000$, $D_{\text{verdade}}=1{,}997$). O padrão é consistente e sem exceções: nos dois sinais e nos três tamanhos, \texttt{direct} obtém sempre o menor $D_{\text{verdade}}$ e \texttt{poisson} sempre o maior --- as três variantes \texttt{poisson} ocupam, em cada sinal, as três \emph{últimas} posições entre as 150 configurações avaliadas.

Uma explicação consistente com os dados é que $\alpha$ decresce monotonicamente com a quantidade de ruído estocástico injetado pela codificação. A variante \texttt{direct} não injeta ruído algum (passagem analógica) e atinge $\alpha=0{,}667$, praticamente igual ao do ANN-AE de mesmo tamanho ($\alpha=0{,}643$) --- retirados os pulsos, o SNN-AE se comporta como o \textit{autoencoder} denso. A \texttt{latency} injeta quantização determinística ($\alpha=0{,}116$), e a \texttt{poisson} injeta amostragem aleatória a cada quadro, colapsando a $\alpha=0{,}000$, ou seja, exatamente sobre o vértice \textit{Falsidade} do plano paraconsistente. Como $\alpha$ é definido a partir da \emph{amplitude} (mínimo--máximo) intraclasse (\autoref{sec:conceitos}), basta que a estocasticidade produza poucas amostras extremas para que a amplitude de uma classe se estenda por todo o intervalo normalizado e $\alpha \to 0$; com $T=16$ quadros, a estimativa da taxa de disparo da \texttt{poisson} tem variância alta demais para evitar esse efeito.

Cabe uma ressalva: $D_{\text{verdade}}$ mede separabilidade sob um critério baseado em amplitude e, portanto, sensível a valores extremos --- um $D_{\text{verdade}}$ alto não demonstra, isoladamente, que as características produzidas pela \texttt{poisson} sejam inúteis a um classificador, apenas que são difusas segundo esse critério. Ainda assim, como a Fase~00 seleciona precisamente pelo menor $D_{\text{verdade}}$, as codificações temporais não são eleitas por ele. Registra-se, pois, um \textbf{resultado negativo} em relação à hipótese inicial: neste problema e sob este critério, a codificação temporal não melhorou a separabilidade do latente do SNN-AE --- constatação que motiva, como trabalho futuro, reavaliar $T$ (número de quadros) e um critério de seleção menos sensível a valores extremos antes de descartar a codificação temporal.
...
Cada combinação gera um vetor de características, avaliado pela distância $D_{\text{verdade}}$ --- a distância ao vértice \textit{Verdade} no plano paraconsistente (\autoref{sec:conceitos}); quanto menor, mais separáveis são as classes antes de qualquer classificador. O vencedor do sinal é a combinação de menor $D_{\text{verdade}}$. Nenhum classificador é treinado nesta fase. O \autoref{lst:e05Phase00} descreve o procedimento e a \autoref{fig:e05Phase00Flow} o resume.
		...
		\subsection{Métricas}
			...
			\par A autenticação segue o protocolo de \textbf{verificação} (\textit{verification}), no qual os locutores de teste não aparecem no treino: os grupos de validação cruzada são disjuntos por locutor. Nesse protocolo, as métricas primárias são a \textbf{\ac{EER}} (o ponto em que a taxa de falsa aceitação iguala a de falsa rejeição) e a \textbf{área sob a curva \ac{ROC}} (\ac{AUC}). As métricas de classificação de conjunto fechado (acurácia, F1, precisão, recall, especificidade) só se aplicam quando as classes de teste também estão no treino; sob o protocolo de verificação elas não são reportadas. As seguintes métricas serão adotadas, garantindo a comparação com outros estudos:
