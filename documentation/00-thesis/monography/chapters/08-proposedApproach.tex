	...
	\section{Estrutura da estratégia proposta}
		...
		\subsection{Protocolo experimental em duas fases}
			...
			\subsubsection{Fase 00 --- construção e seleção do vetor de características}
				...
				\par Considerando os dois sinais, a Fase 00 tem $2 \times (69 + 3) = 144$ execuções de ranking. O vetor fundido não é varrido aqui: ele é montado na Fase 01, a partir dos vencedores de cada sinal.

			\subsubsection{Fase 01 --- autenticação com o vetor vencedor}

				\par A Fase 01 usa \emph{apenas} a combinação vencedora da Fase 00 --- o eixo do extrator não é mais variado, pois a engenharia paraconsistente já o fixou. Variam-se três eixos: a \textbf{fonte} do sinal (apenas voz, apenas EEG, fundida-precoce, fundida-tardia), o \textbf{modo de texto} (dependente ou independente) e o \textbf{esquema de validação cruzada} (aninhada ou plana). São $4 \times 2 \times 2 = 16$ execuções, todas com o classificador DSNN. O \autoref{lst:e05Phase01} mostra a montagem da fonte e o laço de autenticação; a \autoref{fig:e05Phase01Flow} resume o fluxo.
			...
			\subsubsection{Organização dos perfis e transição entre fases}
				...
				\par Cada execução é descrita por um \textit{perfil} em formato JSON. Os perfis ficam separados por fase: 144 perfis em \texttt{profiles/phase00/} (um por combinação varrida) e 16 em \texttt{profiles/phase01/} (um por fonte $\times$ texto $\times$ CV). O vencedor da Fase 00 só é conhecido após a execução; por isso os perfis da Fase 01 nascem com um extrator provisório, substituído pelo vencedor antes de rodar.

				\par Dois \textit{scripts} automatizam a transição, evitando edição manual (\autoref{fig:e05PipelineTwoPhase}):
				\begin{itemize}
					\item \texttt{e05\_phase00\_rank.py}: lê os resultados paraconsistentes das 144 execuções, calcula a média de $D_{\text{verdade}}$ sobre as repetições e escreve o vencedor de cada sinal em \texttt{winners.json};
