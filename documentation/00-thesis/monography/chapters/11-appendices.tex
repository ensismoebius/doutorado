...
%
	...
	\section{Decaimento de Peso (Regularização L2)}
		...
		\par A ideia é simples: em vez de modificar o gradiente, encolhe-se cada peso por uma pequena fração a cada passo de treinamento, logo após a atualização normal do otimizador:
		\begin{equation}
			\theta \leftarrow \theta - \eta\, \lambda\, \theta,
		\end{equation}
		em que $\eta$ é a taxa de aprendizado e $\lambda$ controla a força do decaimento. Em outras palavras, a cada iteração o peso perde uma fração $\eta\lambda$ do seu valor. Por exemplo, com $\eta = 0{,}01$ e $\lambda = 0{,}1$, cada peso é multiplicado por $(1 - 0{,}001) = 0{,}999$ a cada passo, sendo lentamente puxado em direção a zero. Pesos realmente úteis recebem gradientes que compensam esse encolhimento e sobrevivem; pesos irrelevantes, que quase não recebem gradiente, definham até desaparecer.
	...
	\section{Regularização da Taxa de Disparo em Redes Neurais de Pulso}
		...
		\par A solução é tratar a taxa de disparo como algo que deve ficar dentro de uma faixa saudável $[r_{\min}, r_{\max}]$ --- nem baixa demais, nem alta demais. Adiciona-se à perda uma penalidade que só ``acende'' quando a taxa média $r_\ell$ de uma camada sai dessa faixa, funcionando como duas cercas elásticas que a empurram de volta:
		\begin{equation}
		...
		\end{equation}
		em que $r_\ell$ é a média dos pulsos ($0$ ou $1$) da camada $\ell$ sobre todos os neurônios e instantes de tempo, e $\lambda$ regula a intensidade da penalidade. Enquanto $r_\ell$ estiver dentro da faixa, os dois termos $\max(0, \cdot)$ valem zero e nada é penalizado; fora dela, a penalidade cresce com o quadrado da distância até a borda.
