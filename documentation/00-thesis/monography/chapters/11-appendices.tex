...
%
	...
	\section{Decaimento de Peso (Regularização L2)}
		...
		\par Suponha que tenhamos uma rede neural simples com pesos \( W \). Sejam $L_{\text{dados}}$ a perda de dados (por exemplo, erro quadrático médio para tarefas de regressão), $\lambda$ o parâmetro de regularização e $\|W\|_2^2$ a norma L2 dos pesos. Durante o treinamento, adicionamos um termo de regularização à função de perda, que penaliza o quadrado dos pesos \cite{krogh1991simple}:
		\begin{equation}
		...
		\par O exemplo acima soma a penalidade diretamente à perda, que é a forma clássica da regularização L2. Neste trabalho usa-se uma forma equivalente, porém mais estável quando combinada com o otimizador Adam: o \textbf{decaimento de peso desacoplado} (\textit{decoupled weight decay}), proposto por \cite{loshchilov2019decoupled} no algoritmo AdamW.

		\par A ideia é simples: em vez de modificar o gradiente, encolhe-se cada peso por uma pequena fração a cada passo de treinamento, logo após a atualização normal do otimizador. Sejam $\theta$ um peso qualquer da rede, $\eta$ a taxa de aprendizado e $\lambda$ o parâmetro que controla a força do decaimento:
		\begin{equation}
		...
		\par Por que separar o encolhimento do gradiente? O Adam divide cada gradiente por uma estimativa da sua própria escala (o fator $1/\sqrt{\hat{v}}$). Se a penalidade fosse somada ao gradiente, ela também seria dividida por esse fator e o decaimento ficaria mais forte para alguns pesos e mais fraco para outros. Mantendo o encolhimento à parte, todos os pesos diminuem na mesma proporção.

		\par Um cuidado importante: o decaimento é aplicado \textbf{apenas às matrizes de peso} das camadas densas. Os vieses (\textit{bias}) e os parâmetros biofísicos dos neurônios de pulso --- resistência $R$, capacitância $C$ e limiar de disparo $V_{th}$ --- ficam de fora. O motivo é direto: esses valores não são ``pesos'' a serem reduzidos. Empurrar $R$ ou $C$ para zero destruiria a constante de tempo da membrana $\tau = R\cdot C$, e empurrar $V_{th}$ para zero faria o neurônio disparar o tempo todo.
	...
	\section{Penalidade de Esparsidade}
		...
		\par Segundo \cite{ng2011sparse}, o termo de penalidade de esparsidade pode ser definido como a divergência Kullback-Leibler (KL) entre o nível de esparsidade desejado \( \rho \) e $\hat{\rho}_i$, a média da ativação da unidade oculta $i$, somada sobre todas as unidades ocultas:
	...
	\section{Regularização da Taxa de Disparo em Redes Neurais de Pulso}
		\label{Apend:firingRateReg}
		\par A penalidade de taxa de disparo descrita nesta seção é uma técnica \textbf{proposta neste trabalho} para o treinamento das RNPs do \autoref{ch:revisao} --- não é uma reprodução de uma técnica publicada, mas uma adaptação da ideia geral de penalidade de esparsidade (seção anterior) ao caráter binário e temporal dos neurônios de pulso.

		\par Diferentemente de um neurônio clássico, que produz um número real, um neurônio de pulso só pode emitir $0$ ou $1$ a cada instante. A informação que ele transmite está, portanto, na \textbf{frequência} com que dispara ao longo do tempo --- a chamada \textit{taxa de disparo}. Um detalhe simples ilustra o problema: se um neurônio quase nunca dispara, ou se dispara a toda hora, ele praticamente não distingue um exemplo de outro. Surgem assim dois extremos que prejudicam o aprendizado:
		\begin{itemize}
			\item \textbf{Neurônios mortos} (\textit{dead neurons}): a taxa de disparo cai para perto de zero. O neurônio para de contribuir para a saída. Pior: como o gradiente substituto só existe nas proximidades do limiar, deixa de chegar gradiente ao neurônio, que nunca mais é ajustado e fica ``preso'' inativo.
			\item \textbf{Neurônios saturados} (\textit{bursting neurons}): a taxa de disparo sobe para perto de um. O neurônio dispara em quase todo instante, responde igual a qualquer entrada (perde seletividade) e ainda gasta energia à toa --- o oposto da economia que motiva o uso de RNP.
		\end{itemize}

		\par A solução é tratar a taxa de disparo como algo que deve ficar dentro de uma faixa saudável $[r_{\min}, r_{\max}]$ --- nem baixa demais, nem alta demais. Seja $r_\ell$ a média dos pulsos ($0$ ou $1$) da camada $\ell$ sobre todos os neurônios e instantes de tempo, e $\lambda$ o parâmetro que regula a intensidade da penalidade. Adiciona-se à perda uma penalidade que só ``acende'' quando $r_\ell$ sai da faixa saudável, funcionando como duas cercas elásticas que a empurram de volta:
		\begin{equation}
			\label{eq:firingRateReg}
			\Omega_{\text{disparo}} = \lambda \sum_{\ell\,\in\,\text{camadas}} \Big( \underbrace{\max(0,\; r_{\min} - r_\ell)^2}_{\text{pune taxa baixa}} + \underbrace{\max(0,\; r_\ell - r_{\max})^2}_{\text{pune taxa alta}} \Big),
		\end{equation}
		Enquanto $r_\ell$ estiver dentro da faixa, os dois termos $\max(0, \cdot)$ valem zero e nada é penalizado; fora dela, a penalidade cresce com o quadrado da distância até a borda.

		\par Um exemplo torna isso concreto. Sejam $r_{\min}=0{,}05$, $r_{\max}=0{,}80$ e $\lambda = 0{,}01$. Para uma camada com taxa média $r_\ell = 0{,}02$ (quase morta), apenas o primeiro termo atua:
		\begin{equation}
			\Omega = 0{,}01 \times (0{,}05 - 0{,}02)^2 = 0{,}01 \times 0{,}0009 = 9 \times 10^{-6}.
		\end{equation}
		Já uma camada com $r_\ell = 0{,}40$, dentro da faixa, recebe penalidade nula. Sejam $s_j$ o pulso ($0$ ou $1$) do $j$-ésimo neurônio-instante da camada, $n$ o número total desses elementos ($n$ neurônios $\times$ passos de tempo) e $\operatorname{clamp}(r_\ell, r_{\min}, r_{\max})$ a função que satura $r_\ell$ nos limites $r_{\min}$ e $r_{\max}$. O gradiente dessa penalidade em relação a cada pulso é
		\begin{equation}
			\frac{\partial \Omega_{\text{disparo}}}{\partial s_j} = \frac{2\lambda}{n}\big(r_\ell - \operatorname{clamp}(r_\ell, r_{\min}, r_{\max})\big),
		\end{equation}
		\par \textbf{Exemplo numérico.} Continuando o caso $r_\ell=0{,}02$ (quase morta) com $\lambda=0{,}01$, para uma camada de $n=20$ elementos (por exemplo, $4$ neurônios $\times$ $5$ passos de tempo): $\operatorname{clamp}(0{,}02,\,0{,}05,\,0{,}80)=0{,}05$ (satura no limite inferior), logo $\frac{\partial \Omega_{\text{disparo}}}{\partial s_j} = \frac{2\times0{,}01}{20}(0{,}02-0{,}05) = 0{,}001\times(-0{,}03) = -3\times10^{-5}$. O gradiente negativo empurra $s_j$ para cima durante a atualização (que subtrai o gradiente), ou seja, incentiva mais disparos --- consistente com a camada estar abaixo de $r_{\min}$.
	...
	\begin{landscape}
		...
		\begin{figure}[H]
			\centering
			\caption[Demonstração numérica das transformadas Wavelet]{Demonstração numérica das transformadas Wavelet e \textit{wavelet-packet}: Por razões didáticas, a \textit{Wavelet} Haar foi considerada como tendo os valores $1/2$ e $-1/2$}
