...
## Two Other Implementations of the Same Math (Experiment05)

`SpikeCountLossImpl` above is a reusable **loss-layer** form: it fits when the regularized spike tensor *is* the loss's own prediction. Experiment05's classifier and autoencoder don't fit that shape — their loss (cross-entropy / MSE) is computed on a later, non-spiking output (class logits / decoded reconstruction), not on the spike tensor itself — so both instead inject the identical gradient term directly into the backward pass, at the point where it reaches each spiking layer's output, rather than adding it inside a loss module:

- **`ThesisDsnnClassifier::add_firing_rate_grad`** (`src/experiments/thesis/lib/src/ThesisClassifiers.cpp`): the classifier is built from named layers (`fc_in_`, `lif_in_`, `hidden_fc_[]`, `hidden_lif_[]`, ...), so the call site is explicit — `add_firing_rate_grad(g, spikes_hidden_[i])` right before `g = hidden_lif_[i]->backward(g)`. Config: `training.firing_rate_reg_lambda/min/max`.
- **`ProtocolSpikingAutoencoder`'s `backward_with_firing_rate_reg`** (`src/experiments/autoencoderRunner/lib/src/autoencoder/ProtocolSpikingAutoencoder.cpp`, added 2026-07-16 as part of the D1 dead-latent fix): the AE's encoder is instead a generically-built `Sequential` (assembled at runtime from `encoder_layer_spec` strings), so there's no named layer to hook. The constructor locates every `Lif` layer's index once via `dynamic_cast<nn::Lif*>` over `Sequential::layers`, and `backward()` replaces the plain `Sequential::backward` call with a manual reversed-order loop that injects the same gradient right before propagating through each located Lif index, without modifying the shared `Sequential` class itself. Config: `feature_extraction.autoencoder.firing_rate_reg_lambda/min/max` (see [Experiment05](../Experiments/Thesis.md#regularization--normalization)).

Both default to `λ=0` (inert). As of 2026-07-16, the DSNN classifier still has no real profile with `λ>0` (only debug/smoke), while all 18 real `snn-ae` Phase 00 profiles (and their 18 smoke mirrors) set `λ=0.5, r_min=0.10, r_max=0.80` — see the Experiment05 wiki page for how those numbers were chosen and their current validation status.
