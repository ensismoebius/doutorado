...
- **`E05DsnnClassifier::add_firing_rate_grad`** (`src/experiments/05/lib/src/E05Classifiers.cpp`): the classifier is built from named layers (`fc_in_`, `lif_in_`, `hidden_fc_[]`, `hidden_lif_[]`, ...), so the call site is explicit — `add_firing_rate_grad(g, spikes_hidden_[i])` right before `g = hidden_lif_[i]->backward(g)`. Config: `training.firing_rate_reg_lambda/min/max`.
- **`ProtocolSpikingAutoencoder`'s `backward_with_firing_rate_reg`** (`src/experiments/03/lib/src/autoencoder/ProtocolSpikingAutoencoder.cpp`, added 2026-07-16 as part of the D1 dead-latent fix): the AE's encoder is instead a generically-built `Sequential` (assembled at runtime from `encoder_layer_spec` strings), so there's no named layer to hook. The constructor locates every `Lif` layer's index once via `dynamic_cast<nn::Lif*>` over `Sequential::layers`, and `backward()` replaces the plain `Sequential::backward` call with a manual reversed-order loop that injects the same gradient right before propagating through each located Lif index, without modifying the shared `Sequential` class itself. Config: `feature_extraction.autoencoder.firing_rate_reg_lambda/min/max` (see [Experiment05](../Experiments/Experiment05.md#regularization--normalization)).
