...
| Script | Runs | Params |
|---|---|---|
| `scripts/testing/run_profiles.sh [phase00\|phase01\|all]` | the **real** profiles | full (the actual experiment) |
| `scripts/testing/run_smoke.sh [phase00\|phase01\|all]` | the **smoke** mirror | tiny (fast crash check) |

**Binary selection** (works with any build profile):

- auto: the most recently built `out/build/*/…/experiment05`;
- `E05_BUILD=max-performance-opencl ./scripts/testing/run_profiles.sh phase00`;
...
```bash
./scripts/testing/run_smoke.sh phase00
./scripts/testing/run_smoke.sh phase01
```
...
```bash
# heavy — background it
nohup ./scripts/testing/run_profiles.sh phase00 > phase00_run.log 2>&1 &
```
