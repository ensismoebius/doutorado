...
TEST_F(MatFileTest, LargeMatrix)
{
    ...
    EXPECT_TRUE(mat_file.write_double_matrix("large_matrix", large_data, {100, 10}));
    mat_file.close();
...
TEST_F(MatFileTest, ComplexIntegrationTest)
{
    ...
    ASSERT_TRUE(mat_file.write_double_matrix("double_matrix", double_data, double_dims));

    std::vector<float> float_data = {1.0F, 2.0F, 3.0F, 4.0F};
    Dimensions float_dims = {2, 2};
    ASSERT_TRUE(mat_file.write_single_matrix("float_matrix", float_data, float_dims));
...
TEST_F(MatFileTest, DataPersistence)
{
    // Write data
    {
        ...
        EXPECT_TRUE(mat_file.write_double_matrix("persistent_data", {42.0, 84.0}, {1, 2}));
...
TEST_F(MatFileTest, VariableNameEdgeCases)
{
    ...
    EXPECT_TRUE(mat_file.write_double_matrix("name_with_underscores", {2.0}, {1, 1}));
