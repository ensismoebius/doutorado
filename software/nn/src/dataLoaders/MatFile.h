...
// Variable data type
using MatData =
    std::variant<std::vector<double>, std::vector<float>, std::vector<int8_t>, std::vector<uint8_t>,
                 std::vector<int16_t>, std::vector<uint16_t>, std::vector<int32_t>,
                 std::vector<uint32_t>, std::vector<int64_t>, std::vector<uint64_t>, std::string>;
...
class MatFile
{
   private:
    ...
    void read_and_store_matrix_variable(std::unordered_map<std::string, MatVar>& variables);
    ...
    auto process_top_level_element(const DataTag& tag,
                                   std::unordered_map<std::string, MatVar>& variables) -> bool;
    // Helper to parse a decompressed MI_COMPRESSED buffer
    void parse_decompressed_buffer(const std::vector<uint8_t>& buf,
    ...
    auto read_numeric_from_buffer(const std::vector<uint8_t>& buf, size_t& pos, DataType dtype,
   ...
   public:
    ...
    auto write_double_matrix(const std::string& name, const std::vector<double>& data,
                             const Dimensions& dims = {1, 1}) -> bool;
    auto write_single_matrix(const std::string& name, const std::vector<float>& data,
                             const Dimensions& dims = {1, 1}) -> bool;
    auto write_int32_matrix(const std::string& name, const std::vector<int32_t>& data,
                            const Dimensions& dims = {1, 1}) -> bool;
