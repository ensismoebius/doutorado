...
void MatFile::read_and_store_matrix_variable(unordered_map<string, MatVar>& variables)
{
    MatVar var;
    // Read array flags
...
auto MatFile::read_numeric_from_buffer(const std::vector<uint8_t>& buf, size_t& pos, DataType dtype,
...
auto ArrayFlags::is_complex() const -> bool
{
    return (flags & 0x08) != 0U;
}

auto ArrayFlags::is_global() const -> bool
{
    return (flags & 0x04) != 0U;
}

auto ArrayFlags::is_logical() const -> bool
{
    return (flags & 0x02) != 0U;
}
...
auto MatFile::read_numeric_data(DataType data_type, uint32_t num_bytes) -> MatData
{
    ...
    switch (data_type)
    {
        ...
        case DataType::MI_INT8:
        {
            ...
            file_.read(reinterpret_cast<char*>(data.data()), static_cast<long>(num_elements));
            result = data;
            break;
        }
        case DataType::MI_UINT8:
...
{
    write_tag(DataType::MI_INT32, static_cast<uint32_t>(dims.size() * sizeof(int32_t)));
    for (auto dim : dims)
    {
    ...
    std::visit(
        [this, &data](auto&& arg)
        {
            ...
            else
            {
                ...
                auto num_bytes = static_cast<uint32_t>(arg.size() * sizeof(typename T::value_type));
...
auto MatFile::read_all_variables() -> std::unordered_map<std::string, MatVar>
{
    ...
    catch (const std::exception& e)
    {
        ...
        throw std::runtime_error(std::string("Error reading variables: ") + e.what());
    }

    return variables;
}

auto MatFile::process_top_level_element(const DataTag& tag,
                                        std::unordered_map<std::string, MatVar>& variables) -> bool
{
...
void MatFile::parse_decompressed_buffer(const std::vector<uint8_t>& buf,
                                        std::unordered_map<std::string, MatVar>& variables)
{
    size_t pos = 0;
...
auto MatFile::write_variable(const std::string& name, const MatData& data, const Dimensions& dims,
                             ArrayFlags flags) -> bool
{
    if (!file_.is_open())
    {
        return false;
...
auto MatFile::write_double_matrix(const std::string& name, const std::vector<double>& data,
                                  const Dimensions& dims) -> bool
{
    return write_variable(name, data, dims, {.array_type = ArrayType::MX_DOUBLE_CLASS, .flags = 0});
}

auto MatFile::write_single_matrix(const std::string& name, const std::vector<float>& data,
                                  const Dimensions& dims) -> bool
{
    return write_variable(name, data, dims, {.array_type = ArrayType::MX_SINGLE_CLASS, .flags = 0});
}

auto MatFile::write_int32_matrix(const std::string& name, const std::vector<int32_t>& data,
                                 const Dimensions& dims) -> bool
{
    return write_variable(name, data, dims, {.array_type = ArrayType::MX_INT32_CLASS, .flags = 0});
}
