/**
 * @file src/demos/cppDemos/snn_speaker_demo/comandos.cpp
...
// Builds an MFCC-style pipeline config from the demo's window/hop/band parameters and runs
// pre-emphasis -> framing/windowing -> FFT power -> filterbank -> DCT to produce a
// (num_frames, num_bands) feature matrix.
auto extract_feature_windows(
    std::vector<float>& audio, int sample_rate, int window_size, int hop_size, int num_bands)
    -> nn::Tensor
{
...
// Tiny Linear -> Lif -> Linear SNN, Kaiming-initialized, that the demo runs the encoded
// spike train through.
auto create_snn_model(int in_dim, int num_bands) -> std::shared_ptr<Sequential>
{
...
// Poisson-encodes each feature frame into `steps_per_window` spike steps, runs them through
// `model`, and accumulates the per-frame output spikes.
auto run_inference(const std::shared_ptr<Sequential>& model,
    const nn::Tensor& features,
    int steps_per_window,
    std::mt19937& rng) -> std::vector<nn::Tensor>
{
...
// Writes the per-frame spike accumulators as a CSV and the synthesized audio as a WAV file.
void write_demo_outputs(const std::string& outpath,
...
)
{
    ...
    // --- main pipeline using the helper functions above ---
    ...
    auto features = extract_feature_windows(audio, sample_rate, window_size, hop_size, num_bands);
    int in_dim = static_cast<int>(features.cols());
    auto model = create_snn_model(in_dim, num_bands);
    auto output_spike_frames = run_inference(model, features, steps_per_window, rng);
