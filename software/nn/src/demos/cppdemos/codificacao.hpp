 ...
 */
float compute_adaptive_max_rate(const nn::Tensor& signal, float expected_spikes_per_step = 0.10f,
                                float min_max_rate = 0.02f, float max_max_rate = 0.50f,
                                float epsilon = 1e-8f);
...
inline float calcular_taxa_max_adaptativa(const nn::Tensor& sinal,
                                          ...
                                          float frequencia_max_max = 0.50f, float eps = 1e-8f)
{
 ...
 */
nn::Tensor encode_poisson(const nn::Tensor& frame, int time_steps, std::mt19937& random_engine,
                          float max_rate = -1.0f, bool adaptive_rate = true,
                          float expected_spikes_per_step = 0.10f);

// Backward-compatible alias for existing callers.
inline nn::Tensor codificar_poisson(const nn::Tensor& frm, int passos, std::mt19937& rng,
                                    float frequencia_max = -1.0f, bool adaptativo = true,
                                    float qtde_de_spikes_esperada_por_passo = 0.10f)
{
