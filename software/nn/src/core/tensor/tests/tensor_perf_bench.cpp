...
auto benchmark_backend(const std::string& backend_name, int iterations) -> std::vector<BenchResult>
{
    ...
    results.push_back(measure(
        backend_name, "random_1024x1024", 1024, 1024, 0, iterations, [&rng]() mutable {
            auto tensor = Tensor::random(1024, 1024, rng);
            volatile float sink = tensor.at(0, 0);
            (void) sink;
        }));

    results.push_back(measure(backend_name, "fill_zero_1024x1024", 1024, 1024, 0, iterations, [&]() {
        auto tensor = Tensor::zeros(1024, 1024);
        volatile float sink = tensor.at(0, 0);
        (void) sink;
    }));

    results.push_back(measure(backend_name, "matmul_256", 256, 256, 256, iterations, [&]() {
        auto product = lhs.matmul(rhs);
        volatile float sink = product.at(0, 0);
        (void) sink;
    }));

    results.push_back(measure(backend_name, "linear_chain_1024x256x512", 1024, 512, 256, iterations, [&]() {
