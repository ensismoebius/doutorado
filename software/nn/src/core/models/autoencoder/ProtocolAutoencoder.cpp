...
ProtocolAutoencoder::ProtocolAutoencoder(const AutoencoderConfig& cfg)
{
    ...
    if (use_dual_branch_)
    {
        ...
        eeg_encoder_ = build_ann_encoder(cfg, cfg.eeg_features, resolved_branch_hidden_size(cfg));
        audio_encoder_ =
            build_ann_encoder(cfg, cfg.audio_features, resolved_branch_hidden_size(cfg));
        fusion_encoder_ =
            build_ann_encoder(cfg, cfg.latent_size * 2, resolved_fusion_hidden_size(cfg));
        fusion_decoder_ =
            build_ann_decoder(cfg, cfg.latent_size * 2, resolved_fusion_hidden_size(cfg));
        eeg_decoder_ = build_ann_decoder(cfg, cfg.eeg_features, resolved_branch_hidden_size(cfg));
        audio_decoder_ =
            build_ann_decoder(cfg, cfg.audio_features, resolved_branch_hidden_size(cfg));
    }
    else
    {
        encoder_ = build_ann_encoder(cfg, cfg.input_features, cfg.hidden_size);
        decoder_ = build_ann_decoder(cfg, cfg.input_features, cfg.hidden_size);
    }
}
