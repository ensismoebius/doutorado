...
inline auto make_random_tensor(nn::Index rows, nn::Index cols, float lower = -1.0F,
                               float upper = 1.0F) -> nn::Tensor
{
...
inline auto make_constant_tensor(nn::Index rows, nn::Index cols, float value) -> nn::Tensor
{
    nn::Tensor t(rows, cols);
    for (nn::Index i = 0; i < rows; ++i)
    {
...
inline auto make_ones_tensor(nn::Index rows, nn::Index cols) -> nn::Tensor
{
    return make_constant_tensor(rows, cols, 1.0F);
}

inline auto make_zeros_tensor(nn::Index rows, nn::Index cols) -> nn::Tensor
{
...
inline void tensor_set_value_at(nn::Tensor& t, nn::Index row, nn::Index col, float value)
{
    t.at(row, col) = value;
}
