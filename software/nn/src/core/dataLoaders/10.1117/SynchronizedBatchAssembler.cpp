    ...
    nn::Tensor& targets)
{
    ...
    while (eeg_block_pos < eeg_sorted_task_indices.size())
    {
        ...
        for (size_t j = 0; j < eeg_run_count; ++j)
        {
            ...
            // Default behavior: write raw EEG (flattened per-channel) then audio.
...
// Variant of processEegBlocksForTasks that produces stacked, channel-preserving
...
template <typename AudioRowsFlatT>
static void processEegBlocksForTasksStacked(size_t subject_index,
    const SubjectFiles& subject,
    const std::vector<std::unique_ptr<nn::dataLoaders::EEGMatSession>>& eeg_sessions,
    const AudioRowsFlatT& audio_rows_flat,
    std::vector<BatchTask>& tasks,
    ...
    nn::Tensor& targets)
{
    ...
    while (eeg_block_pos < eeg_sorted_task_indices.size())
    {
        ...
        for (size_t j = 0; j < eeg_run_count; ++j)
        {
            ...
            const size_t flat_width = stacked_rows * stacked_cols;
...
void SynchronizedBatchAssembler::assembleGroupedStacked(                                  //
...
)
{
    for (size_t subject_index = 0; subject_index < grouped.size(); ++subject_index)
    {
        ...
        while (audio_run_pos < requests.size())
        {
            ...
            processEegBlocksForTasksStacked( //
                subject_index,               //
                subject,                     //
                eeg_sessions,                //
                audio_rows_flat,             //
