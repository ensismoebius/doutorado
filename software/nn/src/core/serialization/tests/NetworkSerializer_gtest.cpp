...
TEST(NetworkSerializerTest, SaveLoadRoundTripMatchesPyTorchStandard)
{
    ...
    model.layers.push_back(std::make_shared<Lif>(1.0F, 2.0F, 3.0F, 4.0F, true, 0.5F));
    ...
    auto leaky = std::dynamic_pointer_cast<Lif>(model.layers[2]);
    ...
    // Load into a new model and validate full round-trip.
    Sequential loaded;
    ...
    EXPECT_TRUE(std::dynamic_pointer_cast<Lif>(loaded.layers[2]));
    ...
    // Check Lif config and parameters
    auto loaded_leaky = std::dynamic_pointer_cast<Lif>(loaded.layers[2]);
    ASSERT_TRUE(loaded_leaky);
    EXPECT_FLOAT_EQ(loaded_leaky->delta_t, 1.0F);
