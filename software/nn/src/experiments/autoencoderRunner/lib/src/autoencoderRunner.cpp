...
// Core libraries
#include "Backend.hpp"
#include "data_loaders/10.1117/datasets/raw/Dataset101117Printer.hpp"
...
void AutoencoderRunner::initialize_dataset()
{
    ////////////////////////////
    ...
    {
        ostringstream _oss;
        _oss << "Warning: selected dataset type '" << dataset_type_to_string(config_.dataset_type)
...
bool AutoencoderRunner::build_and_attach_model(const nn::Device& device, Optimizer& optimizer)
{
...
auto AutoencoderRunner::setup_fold_selector(std::vector<int>& test_trial_ids) const
    -> autoencoderRunner::TrialFoldSelector
{
    ...
    try
    {
        fold_selector = autoencoderRunner::TrialFoldSelector::from_sqlite(config_.dataset_root_path,
...
auto AutoencoderRunner::train_one_epoch(Optimizer& optimizer,
    ...
    bool also_update_member_counters) -> float
{
    unique_ptr<IBatchSource> train_src = make_unique<SqliteBatchSource>( //
        ...
        trial_ids != nullptr ? *trial_ids : vector<int>{}                //
    );
    ...
    while (prefetcher_->hasNext())
    {
        ...
        ++epoch_batches;
        progress_processed_samples += static_cast<size_t>(batch.inputs.rows());
        ++progress_seen_batches;
        // In k-fold mode the lifetime (member) counters are tracked in addition to the
        // per-run "global" progress counters passed in above; in no-kfold mode the members
        // *are* the progress counters (passed by reference), so no separate update is needed.
        if (also_update_member_counters)
        {
            processed_samples_ += static_cast<size_t>(batch.inputs.rows());
            ++seen_batches_;
        }

        postProgressAsync(progress_total_samples,
            config_.training_batch_size,
            progress_total_batches,
            progress_seen_batches,
            progress_processed_samples,
            false,
            fold_number_1based,
            fold_count,
            epoch_index_0based + 1,
            total_epochs,
            epoch_batches,
            epoch_max_batches,
            last_batch_loss,
    ...
    // NOTE: the original (pre-extraction) code had an inert `if (model_) [[likely]] { ... }`
    // block here, with an empty/no-op body in both call sites (k-fold and no-k-fold) — no
    // observable side effects. Elided as dead code; see git history for the exact prior text.

    return epoch_batches > 0 ? epoch_loss_sum / static_cast<float>(epoch_batches) : 0.0F;
}

auto AutoencoderRunner::validate_one_epoch(autoencoderRunner::ReconstructionLoss& loss,
    const std::shared_ptr<nn::transforms::ITransform>& input_transform,
    const std::vector<int>* trial_ids,
    std::size_t max_batches) -> EpochValidationResult
{
    ...
    const size_t audio_features =
        static_cast<size_t>(std::max(0, config_.effective_autoencoder_audio_features()));
    while (val_prefetcher->hasNext())
    {
        ...
        {
            const auto [batch_eeg_loss, batch_audio_loss] = modality_val_losses_from_batch(
    ...
    EpochValidationResult result;
    result.mean_loss = val_batches > 0 ? val_loss_sum / static_cast<float>(val_batches) : 0.0F;
    result.mean_eeg_loss =
        val_batches > 0 ? val_eeg_loss_sum / static_cast<float>(val_batches) : 0.0F;
    result.mean_audio_loss =
        val_batches > 0 ? val_audio_loss_sum / static_cast<float>(val_batches) : 0.0F;
    return result;
}

void AutoencoderRunner::train_and_validate_one_fold(const nn::Device& device,
    autoencoderRunner::ReconstructionLoss& loss,
    const autoencoderRunner::TrialFoldSelection& selection,
    size_t train_epoch_max_batches,
    size_t val_epoch_max_batches,
    size_t fold_number_1based,
    size_t fold_count,
    size_t global_training_total_samples,
    size_t global_training_total_batches,
    size_t& global_training_seen_batches,
    size_t& global_training_processed_samples,
    vector<float>& epoch_mean_losses,
    vector<float>& fold_epoch_val_losses_this_fold,
    vector<float>& fold_epoch_val_eeg_losses_this_fold,
    vector<float>& fold_epoch_val_audio_losses_this_fold,
    vector<float>& fold_mean_val_losses)
{
    ...
    fold_header << "=== K-Fold " << fold_number_1based << "/" << fold_count
                << "  train_trials=" << train_trial_ids.size()
                << "  val_trials=" << val_trial_ids.size()
                << "  train_epoch_max_batches=" << train_epoch_max_batches
                << "  val_max_batches=" << val_epoch_max_batches << " ===";
    ...
    auto input_transform = fit_input_transform(config_, train_epoch_max_batches, &train_trial_ids);

    // Train for training_epochs on the training subset.
    for (size_t epoch = 0; epoch < config_.training_epochs; ++epoch)
    {
        const float mean_train_loss = train_one_epoch(*fold_optimizer,
            loss,
            input_transform,
            &train_trial_ids,
            train_epoch_max_batches,
            global_training_total_samples,
            ...
            /*also_update_member_counters=*/true);
        epoch_mean_losses.push_back(mean_train_loss);

        // Validate this epoch on the held-out validation subset (no backprop).
        const EpochValidationResult val_result =
            validate_one_epoch(loss, input_transform, &val_trial_ids, val_epoch_max_batches);
        fold_epoch_val_losses_this_fold.push_back(val_result.mean_loss);
        fold_epoch_val_eeg_losses_this_fold.push_back(val_result.mean_eeg_loss);
        fold_epoch_val_audio_losses_this_fold.push_back(val_result.mean_audio_loss);

        const float current_lr = apply_reduce_lr_on_plateau(
            *fold_optimizer, config_, fold_lr_state, val_result.mean_loss);

        ostringstream fold_epoch_log;
        fold_epoch_log << "  fold " << fold_number_1based << "/" << fold_count << " epoch "
                       << (epoch + 1) << "/" << config_.training_epochs
                       << "  train loss: " << mean_train_loss
                       << "  val loss: " << val_result.mean_loss;
        if (config_.validation_modality_diagnostics_enabled)
        {
            fold_epoch_log << "  val_eeg: " << val_result.mean_eeg_loss
                           << "  val_audio: " << val_result.mean_audio_loss;
        }
    ...
    const float fold_mean_val =
        fold_epoch_val_losses_this_fold.empty()
            ? 0.0F
            : std::accumulate(fold_epoch_val_losses_this_fold.begin(),
                  fold_epoch_val_losses_this_fold.end(),
                  0.0F) /
                  static_cast<float>(fold_epoch_val_losses_this_fold.size());
    fold_mean_val_losses.push_back(fold_mean_val);

    ostringstream fold_summary_log;
    fold_summary_log << "  fold " << fold_number_1based << "/" << fold_count
                     << " complete: mean val loss = " << fold_mean_val;
    NN_LOG_INFO(fold_summary_log.str());
}

void AutoencoderRunner::run_kfold_training(const nn::Device& device,
    autoencoderRunner::ReconstructionLoss& loss,
    autoencoderRunner::TrialFoldSelector& fold_selector,
    vector<float>& epoch_mean_losses,
    ...
    float& mean_val_loss)
{
    ...
    auto [fold_plans, global_training_total_batches] =
        build_fold_runtime_plans(fold_selector, config_);
    ...
    for (size_t fold_idx = 0; fold_idx < fold_selector.fold_count(); ++fold_idx)
    {
        train_and_validate_one_fold(device,
            loss,
            fold_plans[fold_idx].selection,
            ...
            global_training_processed_samples,
            epoch_mean_losses,
            fold_epoch_val_losses[fold_idx],
            fold_epoch_val_eeg_losses[fold_idx],
            fold_epoch_val_audio_losses[fold_idx],
...
void AutoencoderRunner::run_no_kfold_training(const nn::Device& /*device*/,
    autoencoderRunner::ReconstructionLoss& loss,
    Optimizer& optimizer,
    vector<float>& epoch_mean_losses,
    vector<vector<float>>& fold_epoch_val_losses,
    ...
    float& mean_val_loss)
{
    ...
    const size_t total_training_samples = total_training_batches * config_.training_batch_size;
    const string fold_progress_context = "Training";
    ...
    for (size_t epoch = 0; epoch < config_.training_epochs; ++epoch)
    {
        const float mean_train_loss = train_one_epoch(optimizer,
            loss,
            input_transform,
            nullptr,
            epoch_max_batches,
            ...
            config_.training_epochs,
            fold_progress_context,
            /*also_update_member_counters=*/false);
        epoch_mean_losses.push_back(mean_train_loss);

        // Validate this epoch using the same protocol and budget in no-kfold mode.
        const EpochValidationResult val_result =
            validate_one_epoch(loss, input_transform, nullptr, epoch_max_batches);
        fold_epoch_val_losses[0].push_back(val_result.mean_loss);
        fold_epoch_val_eeg_losses[0].push_back(val_result.mean_eeg_loss);
        fold_epoch_val_audio_losses[0].push_back(val_result.mean_audio_loss);

        const float current_lr =
            apply_reduce_lr_on_plateau(optimizer, config_, lr_state, val_result.mean_loss);

        ostringstream epoch_log;
        epoch_log << "epoch " << (epoch + 1) << "/" << config_.training_epochs
                  << " train loss: " << mean_train_loss << "  val loss: " << val_result.mean_loss;
        if (config_.validation_modality_diagnostics_enabled)
        {
            epoch_log << "  val_eeg: " << val_result.mean_eeg_loss
                      << "  val_audio: " << val_result.mean_audio_loss;
        }
    ...
    const float single_fold_mean_val =
        single_fold_val_epochs.empty()
            ? 0.0F
            : std::accumulate(single_fold_val_epochs.begin(), single_fold_val_epochs.end(), 0.0F) /
...
void AutoencoderRunner::write_success_summary(Optimizer& optimizer,
    const vector<float>& epoch_mean_losses,
    const vector<vector<float>>& fold_epoch_val_losses,
    const vector<vector<float>>& fold_epoch_val_eeg_losses,
    const vector<vector<float>>& fold_epoch_val_audio_losses,
    const vector<float>& fold_mean_val_losses,
    float mean_val_loss,
    const std::vector<int>& test_trial_ids)
{
    ...
    auto summary = build_run_summary( //
        ...
        fold_mean_val_losses,         //
        mean_val_loss,                //
        optimizer_learning_rate_ptr(optimizer) != nullptr ? *optimizer_learning_rate_ptr(optimizer)
                                                          : config_.training_learning_rate, //
...
void AutoencoderRunner::write_failure_summary(const std::exception& e,
    const vector<float>& epoch_mean_losses,
    const vector<vector<float>>& fold_epoch_val_losses,
    const vector<vector<float>>& fold_epoch_val_eeg_losses,
    const vector<vector<float>>& fold_epoch_val_audio_losses,
...
int AutoencoderRunner::run()
{
    ...
    try
    {
        ...
        if (!build_and_attach_model(device, *optimizer))
        {
            return kExitFailure;
        }
        ...
        autoencoderRunner::TrialFoldSelector fold_selector = setup_fold_selector(test_trial_ids);

        if (config_.kfold_enabled)
        {
            run_kfold_training(device,
                loss,
                fold_selector,
                epoch_mean_losses,
                fold_epoch_val_losses,
        ...
        write_success_summary(*optimizer,
            epoch_mean_losses,
            fold_epoch_val_losses,
            fold_epoch_val_eeg_losses,
            fold_epoch_val_audio_losses,
