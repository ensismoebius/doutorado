...
auto EegWindowSpikingAutoencoder::encode(const Tensor& input, bool requires_grad) -> Tensor
{
    return encoder_.forward(input, requires_grad);
}

auto EegWindowSpikingAutoencoder::decode(const Tensor& latent, bool requires_grad) -> Tensor
{
    return decoder_.forward(latent, requires_grad);
}

auto EegWindowSpikingAutoencoder::forward(const Tensor& input, bool requires_grad) -> Tensor
{
    return decode(encode(input, requires_grad), requires_grad);
}

auto EegWindowSpikingAutoencoder::backward(const Tensor& grad_output) -> Tensor
{
    Tensor grad = decoder_.backward(grad_output);
    return encoder_.backward(grad);
}

auto EegWindowSpikingAutoencoder::params() -> std::span<Tensor*>
{
    param_ptrs_.clear();
