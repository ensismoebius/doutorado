...
FusedWindowAutoencoder::FusedWindowAutoencoder(const AutoencoderConfig& cfg)
    : eeg_encoder_(
          [&cfg]()
          {
              ...
              return experiment03::autoencoders::build_ann_encoder(branch_cfg,
                  cfg.eeg_features,
      ...
      fusion_encoder_(
          [&cfg]()
          {
              ...
              return experiment03::autoencoders::build_ann_encoder(fusion_cfg,
                  cfg.latent_size * 2,
      ...
      fusion_decoder_(
          [&cfg]()
          {
              ...
              return experiment03::autoencoders::build_ann_decoder(fusion_cfg,
                  cfg.latent_size * 2,
                  experiment03::autoencoders::resolved_fusion_hidden_size(cfg));
          }()),
      eeg_decoder_(
          [&cfg]()
          {
              ...
              return experiment03::autoencoders::build_ann_decoder(branch_cfg,
                  cfg.eeg_features,
                  experiment03::autoencoders::resolved_branch_hidden_size(cfg));
          }()),
      audio_decoder_(
...
auto FusedWindowAutoencoder::decode(const nn::Tensor& latent, bool requires_grad) -> nn::Tensor
{
...
auto FusedWindowAutoencoder::forward(const nn::Tensor& input, bool requires_grad) -> nn::Tensor
{
    return decode(encode(input, requires_grad), requires_grad);
}

auto FusedWindowAutoencoder::backward(const nn::Tensor& grad_output) -> nn::Tensor
{
