/**
 * @file comparative_experiment.cpp
...
auto read_csv_signal(const std::filesystem::path& path) -> std::vector<float>
{
...
void zscore_inplace(std::vector<float>& signal)
{
    if (signal.empty()) return;
    const float mean =
        std::accumulate(signal.begin(), signal.end(), 0.0f) / static_cast<float>(signal.size());
    float sq_sum = 0.0f;
    for (float v : signal)
    {
...
auto to_window_tensor(const std::vector<float>& signal, int window_size) -> std::vector<Tensor>
{
    std::vector<Tensor> windows;
    if (window_size <= 0 || signal.empty()) return windows;

    std::size_t offset = 0;
    while (offset < signal.size())
    {
        std::vector<float> window(static_cast<std::size_t>(window_size), 0.0f);
        const std::size_t remaining = signal.size() - offset;
        const std::size_t take =
            std::min<std::size_t>(remaining, static_cast<std::size_t>(window_size));
        std::copy(signal.begin() + static_cast<long>(offset),
            signal.begin() + static_cast<long>(offset + take),
            window.begin());
        zscore_inplace(window);

        Tensor sample(window_size, 1);
        for (int t = 0; t < window_size; ++t)
        {
            sample.at(static_cast<nn::Index>(t), 0) = window[static_cast<std::size_t>(t)];
        }
