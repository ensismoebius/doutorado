...
void init_matrices(Tensor& W, Tensor& U, Tensor& x_t, Tensor& h)
{
...
Timing bench_matmul_xw(const Tensor& x_t, const Tensor& W, int reps)
{
    ...
    return {"x_t @ W^T matmul", elapsed / reps};
}

// Benchmark 2: h @ U^T (secondary matmul)
Timing bench_matmul_hu(const Tensor& h, const Tensor& U, int reps)
{
    auto start = Clock::now();
    for (int r = 0; r < reps; ++r)
    {
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"h @ U^T matmul", elapsed / reps};
}

// Benchmark 3: sigmoid_fast on (1, H)
Timing bench_sigmoid_fast(int B, int H, int iters)
{
    Tensor gate(B, H);
    auto start = Clock::now();
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"sigmoid_fast_tensor (1,32)", elapsed / iters};
}

// Benchmark 4: tanh_fast on (1, H)
Timing bench_tanh_fast(int B, int H, int iters)
{
    Tensor gate(B, H);
    auto start = Clock::now();
    for (int r = 0; r < iters; ++r)
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"tanh_fast_tensor (1,32)", elapsed / iters};
}

// Benchmark 5a: 4 gates via block() + activation (old way)
Timing bench_4gates_block(const Tensor& pre, int B, int H, int iters)
{
    auto start = Clock::now();
    for (int r = 0; r < iters; ++r)
    {
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"4_gates block()+fast", elapsed / iters};
}

// Benchmark 5b: 4 gates via fused block+activation (new way)
Timing bench_4gates_fused(const Tensor& pre, int H, int iters)
{
    auto start = Clock::now();
    for (int r = 0; r < iters; ++r)
    {
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"4_gates fused_block", elapsed / iters};
}

// Benchmark 6a: full timestep — old (block + fast)
Timing bench_full_timestep_old(
    const Tensor& x_t, const Tensor& W, const Tensor& h, const Tensor& U, int B, int H, int reps)
{
    Tensor c(B, H);
    auto start = Clock::now();
    for (int r = 0; r < reps; ++r)
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"full_timestep_old", elapsed / reps};
}

// Benchmark 6b: full timestep — new (fused block+activation)
Timing bench_full_timestep_fused(
    const Tensor& x_t, const Tensor& W, const Tensor& h, const Tensor& U, int B, int H, int reps)
{
    Tensor c(B, H);
    auto start = Clock::now();
    ...
    auto elapsed = std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    return {"full_timestep_fused", elapsed / reps};
}

void print_report(const std::vector<Timing>& results)
{
    std::cout << "\n╔══════════════════════════════════════════════════════╗\n";
...
int main()
{
    ...
    init_matrices(W, U, x_t, h);

    std::vector<Timing> results;
    results.push_back(bench_matmul_xw(x_t, W, reps));
    results.push_back(bench_matmul_hu(h, U, reps));
    results.push_back(bench_sigmoid_fast(B, H, reps * T));
    results.push_back(bench_tanh_fast(B, H, reps * T));
    results.push_back(bench_4gates_block(pre, B, H, reps * T));
    results.push_back(bench_4gates_fused(pre, H, reps * T));
    results.push_back(bench_full_timestep_old(x_t, W, h, U, B, H, reps));
    results.push_back(bench_full_timestep_fused(x_t, W, h, U, B, H, reps));
