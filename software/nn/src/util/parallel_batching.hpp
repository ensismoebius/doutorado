...
struct BatchBufferPool {
  ...
  BatchBufferPool(const std::vector<Tensor> &inputSamples, const std::vector<Tensor> &targets,
      ...
      : batch_size(batch_size), input_rows(inputSamples[0].data.rows()),
        input_cols(inputSamples[0].data.cols()), target_rows(targets[0].data.rows()),
...
#pragma omp parallel for schedule(static)
  for (int j = 0; j < actual_batch_size; ++j) {
    ...
    y_concat.block(j * pool.target_rows, 0, pool.target_rows, pool.target_cols) = targets[idx].data;
  }
