...
namespace {
auto debug(const Batch& batch, const Tensor& y_pred, const Tensor& loss_tensor)
    -> void {
  cout << "Input dimensions: " << batch.inputs.data.rows() << "x"
       << batch.inputs.data.cols() << '\n';
  cout << "Output dimensions: " << y_pred.data.rows() << "x"
       << y_pred.data.cols() << '\n';
...
auto main(int /*argc*/, char* /*argv*/[]) -> int {
  ...
  constexpr float learning_rate =
      0.0001;  // Learning rate for the optimizer - low for stability
  constexpr float target_loss =
      -1.0e-14F;                      // Target loss value for early stopping
  ...
  constexpr int epochs =
      200;  // Number of training epochs in which n_samples is presented
  ...
  constexpr int n_samples =
      10;  // Number of samples for synthetic data the higher the better
  ...
  if (!loaded_weights) {
    std::cerr
        << "Failed to load weights, initializing with Kaiming initialization\n";
  ...
  for (size_t epoch = 0; epoch < epochs; ++epoch) {
    ...
    for (const auto& batch : batches) {
      ...
      // Forward pass through encoder and decoder - Eigen will handle internal
      // parallelization
