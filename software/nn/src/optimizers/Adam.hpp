...
struct Adam : public Optimizer {
  ...
  // lr: taxa de aprendizado, beta1: decaimento do primeiro momento, beta2: decaimento do segundo momento, eps: termo de estabilidade.
  ...
  auto step(std::vector<Tensor *> &paramsList) -> void override {
    ...
    for (size_t i = 0; i < paramsList.size(); ++i) {
      ...
      param.data = (param.data.array() - lr * m_hat.array() / (v_hat.array().sqrt() + eps)).matrix();
    }
  }
  ...
  // Suponha um parâmetro θ = [1.0, 2.0], gradiente g_t = [0.1, -0.2], m e v inicializados em zero.
  // Passo 1 (t=1):
  ...
  // O Adam adapta automaticamente o tamanho do passo para cada parâmetro, acelerando a convergência e tornando o treinamento mais robusto a diferentes escalas de gradiente.
