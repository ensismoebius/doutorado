#!/usr/bin/env bash
# 01_meeting01_run_article_profiles.sh — Experiment 04 full paper pipeline: build → run → aggregate.
#
# Builds the meeting01 binary, runs all four article profiles (LSTM-AE, SNN-dense,
...
# Usage:
#   cd software/nn
#   ./scripts/pipeline/meeting01/01_meeting01_run_article_profiles.sh          # asks which build to use
#   MEETING01_BUILD=max-performance ./scripts/…/01_meeting01_run_article_profiles.sh   # no prompt
#   SKIP_BUILD=1 MEETING01_BUILD=max-performance ./scripts/…            # reuse existing binary
#
...
#
#   MEETING01_BUILD=<preset>  choose non-interactively (skips the prompt; required in CI/pipes)
...
# ── choose the build ─────────────────────────────────────────────────────────
# MEETING01_BUILD wins; otherwise ask on a terminal; otherwise (pipe/CI) use the reference.
if [[ -z "${MEETING01_BUILD:-}" ]]; then
  if [[ -t 0 ]]; then
    mapfile -t _presets < <(
      "$PY" - <<'PY'
import json
...
PY
    ...
    if (( ${#_presets[@]} == 0 )); then
      ...
      MEETING01_BUILD="$REFERENCE_BUILD"
    else
    ...
    for i in "${!_presets[@]}"; do
      ...
      _built=""
      [[ -x "out/build/${_presets[$i]}/src/experiments/meeting01/meeting01" ]] && _built=" [built]"
    ...
    MEETING01_BUILD="${_presets[$((_choice - 1))]}"
    fi
  else
    MEETING01_BUILD="$REFERENCE_BUILD"
    echo "[article-run] non-interactive — defaulting to the reference build ($MEETING01_BUILD)"
  fi
fi

BIN="out/build/${MEETING01_BUILD}/src/experiments/meeting01/meeting01"
echo "[article-run] build: ${MEETING01_BUILD}"
...
if [[ "$MEETING01_BUILD" != "$REFERENCE_BUILD" ]]; then
  echo
  echo "[article-run] ⚠  '${MEETING01_BUILD}' is NOT the reference backend (${REFERENCE_BUILD})."
...
PROFILES=(
  "src/experiments/meeting01/profiles/article-snn-dense.json"
  "src/experiments/meeting01/profiles/article-snn-conv1d.json"
  "src/experiments/meeting01/profiles/article-snn-recurrent.json"
  "src/experiments/meeting01/profiles/article-lstm-ae.json"
)
...
if [[ "${SKIP_BUILD:-0}" == "1" ]]; then
  if [[ ! -x "$BIN" ]]; then
    ...
    echo "[article-run] run once without SKIP_BUILD to configure/build '${MEETING01_BUILD}'." >&2
    exit 1
  fi
  ...
  echo "[article-run] configuring/building ${MEETING01_BUILD}"
  cmake --preset="${MEETING01_BUILD}"
  cmake --build --preset="${MEETING01_BUILD}" -j"$(nproc)" --target meeting01
fi

if [[ ! -x "$BIN" ]]; then
  echo "[article-run] no meeting01 binary at $BIN after build" >&2
  exit 1
fi
...
# the binary via MEETING01_OVERALL; the comparative TUI renders it as a persistent top line above
# its own bars (see Meeting01Experiment). The ETA is work-weighted + EMA-smoothed (see run_eta.sh)
...
for profile in "${PROFILES[@]}"; do
  ...
  export MEETING01_OVERALL="$(printf 'Overall  [%d/%d]  elapsed %s  ETA %s   (%s)' \
      "$_i" "$_total" "$(fmt_hms "$_elapsed")" "$_eta" "$(basename "$profile" .json)")"
  echo "[article-run] profile $_i/$_total: $profile"
  echo "[article-run] $MEETING01_OVERALL"
...
"$PY" scripts/data/npz_to_pytorch.py --models-dir results/meeting01/models || true
