...
Reads every  results/meeting01/<tag>_fold*_per_window_errors.csv  (the append-only
per-window dumps written by the meeting01 binary and by
03_meeting01_pca_mean_baselines.py) and compares each model against each reference at
...
# siena (disk space); both kept here (unknown datasets fall to the sort-order tail anyway)
# so pre-swap result CSVs, if any exist, still sort deterministically.
