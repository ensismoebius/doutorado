...
class TensorImpl
{
   public:
    ...
    // -----------------------------------------------------------------
    auto get_shape() const -> const std::vector<Index>& { return m_backend.shape(); }
    void reshape(const std::vector<Index>& new_shape) { m_backend.reshape(new_shape); }
    [[nodiscard]] auto rows() const noexcept -> Index { return m_backend.rows(); }
    [[nodiscard]] auto cols() const noexcept -> Index { return m_backend.cols(); }
    [[nodiscard]] auto size() const noexcept -> Index { return m_backend.size(); }

    // -----------------------------------------------------------------
    // Element access (Inline)
    // -----------------------------------------------------------------
    auto at(Index i) -> float& { return m_backend.at(i); }
    [[nodiscard]] auto at(Index i) const -> const float& { return m_backend.at(i); }

    auto at(Index row, Index col) -> float& { return m_backend.at(row, col); }
    [[nodiscard]] auto at(Index row, Index col) const -> const float& { return m_backend.at(row, col); }

    auto at(Index d1, Index d2, Index d3, Index d4) -> float& { return m_backend.at(d1, d2, d3, d4); }
    [[nodiscard]] auto at(Index d1, Index d2, Index d3, Index d4) const -> const float& { return m_backend.at(d1, d2, d3, d4); }

    auto at(const std::vector<Index>& indices) -> float& { return m_backend.at(indices); }
    [[nodiscard]] auto at(const std::vector<Index>& indices) const -> const float& { return m_backend.at(indices); }

    // -----------------------------------------------------------------
    // Views / Slicing (Backend returns Value)
    // -----------------------------------------------------------------
    auto row(Index i) const -> TensorImpl { return TensorImpl(m_backend.row(i)); }
    auto col(Index j) const -> TensorImpl { return TensorImpl(m_backend.col(j)); }
    auto leftCols(Index n) const -> TensorImpl { return TensorImpl(m_backend.leftCols(n)); }
    auto topRows(Index n) const -> TensorImpl { return TensorImpl(m_backend.topRows(n)); }
    ...
    auto norm() const -> float { return m_backend.norm(); }
    auto sum() const -> float { return m_backend.sum(); }
    
    auto sum_rows() const -> TensorImpl { return TensorImpl(m_backend.sum_rows()); }
    auto sum_cols() const -> TensorImpl { return TensorImpl(m_backend.sum_cols()); }

    auto hasNaN() const -> bool { return m_backend.hasNaN(); }

    // -----------------------------------------------------------------
    // Convenience elementwise math
    // -----------------------------------------------------------------
    auto sqrt() const -> TensorImpl { return TensorImpl(m_backend.sqrt()); }
    auto square() const -> TensorImpl { return TensorImpl(m_backend.square()); }
    auto abs() const -> TensorImpl { return TensorImpl(m_backend.abs()); }
    ...
    // -----------------------------------------------------------------
    void fill(float value) { m_backend.fill(value); }
    void set_zero() { m_backend.set_zero(); }
    void set_ones() { m_backend.set_ones(); }
    void setZero() { set_zero(); }
    void setOnes() { set_ones(); }
    void setConstant(float value) { fill(value); }

    // -----------------------------------------------------------------
    // Raw data access
    // -----------------------------------------------------------------
    const float* data() const { return data_ptr(); }
    float* mutable_data() { return mutable_data_ptr(); }
    
    float& operator()(Index i, Index j) { return at(i, j); }
    const float& operator()(Index i, Index j) const { return at(i, j); }

    const float* data_ptr() const { return m_backend.data_ptr(); }
    float* mutable_data_ptr() { return m_backend.mutable_data_ptr(); }
    ...
    {
        // To allow `tensor.grad() = ...` or modification
        // But our design usually propagates gradient assignment via set_grad/backward.
        // For optimizers: `p->grad()` access.
        // If we want mutable access to Gradient tensor.
        // Implicitly: get_grad in Backend returns a COPY usually?
        // If we need ref: `m_backend.get_grad_ref()`?
        // Let's implement copy for now.
        return TensorImpl(m_backend.get_grad());
    }
    ...
    void zero_grad() { m_backend.zero_grad(); }

    // -----------------------------------------------------------------
    // Operators
    // -----------------------------------------------------------------
    auto operator+(const TensorImpl& other) const -> TensorImpl { return add(other); }
    auto operator-(const TensorImpl& other) const -> TensorImpl 
    { 
        return TensorImpl(m_backend.subtract(other.m_backend));
    }
    
    auto operator*(const TensorImpl& other) const -> TensorImpl { return multiply(other); }
    
    auto operator*(float scalar) const -> TensorImpl { return multiply_scalar(scalar); }
    auto operator+(float scalar) const -> TensorImpl { return add_scalar(scalar); }
    auto operator-(float scalar) const -> TensorImpl { return add_scalar(-scalar); }
    auto operator/(float scalar) const -> TensorImpl { return divide_scalar(scalar); }

    auto operator==(const TensorImpl& other) const -> bool { return m_backend == other.m_backend; }
    auto operator!=(const TensorImpl& other) const -> bool { return !(*this == other); }
    
    template <typename vector_type>
    [[nodiscard]] auto toVector() const -> std::vector<vector_type> { return {}; } // Placeholder
    ...
    auto get_backend() -> Backend& { return m_backend; }
