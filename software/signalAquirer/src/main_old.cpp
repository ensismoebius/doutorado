    ...
    std::string &errorMessage)
{
    ...
    errorMessage = audioCapture->last_error();
    audioCapture.reset();
    capturingAudio = false;
}
...
void widgets()
{
    ...
    {
        toggleAudioCapture(capturing, audioCapture, audioSamples, errorMessage);
    }
    ...
    // Update audio buffer
    if (capturing && audioCapture)
    {
        auto new_samples = audioCapture->getAvailableSamples();
...
int main()
{
    ...
    app.run(
        ...
        {
            widgets();
            audioVisualization(audioSamples, capturing, 400);
            demoTables();
        });
