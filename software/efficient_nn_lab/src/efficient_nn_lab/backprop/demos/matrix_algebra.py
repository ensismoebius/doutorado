...
class _Board:
    """The running frame state, and the frames snapshotted from it.

    Every phase of the demo mutates ONE dict and photographs it. Keeping the
    dict and the photographs together is what let the phases become separate
    methods: previously `snap` was a closure, so the whole 393-line sequence
    had to live in a single function just to stay in its scope.
    """
    ...
    def snap(self, label: str, explanation: str, equation: str = "", **updates: object) -> None:
        """Apply `updates` to the running state and photograph it as one frame."""
        for key, value in updates.items():
            if key not in self.state:
                # No silent fallbacks (CLAUDE.md): a mistyped field name would
                # otherwise quietly add a field no renderer reads, and the step
                # would just not animate.
                raise KeyError(f"unknown frame field {key!r}")
            self.state[key] = value
        self.frames.append(Frame(label, copy_frame_state(self.state), explanation, equation))


def _n2(value: float) -> str:
    """Two decimals, always signed -- so a column of numbers stays aligned."""
    return f"{float(value):+.2f}"


def _n4(value: float) -> str:
    """Four decimals, for gradients small enough that two would read as zero."""
    return f"{float(value):+.4f}"


def _cell(*pairs) -> np.ndarray:
    """A float array from literal rows, for the per-cell highlight masks."""
...
class MatrixAlgebraDemo(DemoModule):
    ...
    def _build_frames(self) -> list[Frame]:
        ...
        board = _Board(state)
        self._phase_mapping(board, x, w1, w2)
        self._phase_forward(board, x, w1, w2, z1, y1, z2, y2)
        self._phase_error(board, y2, target, loss, diff)
        self._phase_backward(board, x, w2, y1, y2, sp1, sp2, gy2, gz2, gw2, gy1, gz1, gw1)
        self._phase_chain(board, gw1, chain_names, chain_values, chain_partials)
        return build_sequence(board.frames, steps=6)

    def _phase_mapping(self, board: _Board, x, w1, w2) -> None:
        """Phase 1: the same network read twice -- as a graph, and as matrices."""
        ...
        board.snap(
            ...
            equation="x = [x_1, x_2]",
            rv_x=_cell(1.0, 0.0), hl_x=_cell(1.0, 0.0),
            work_text=f"x1 = {_n2(x[0])}  ->  célula 1 do vetor x",
        )
        board.snap(
            ...
            equation="x = [x_1, x_2]",
            rv_x=np.ones(2), hl_x=_cell(0.0, 1.0),
            work_text=f"x2 = {_n2(x[1])}  ->  célula 2 do vetor x\nx = coluna [{_n2(x[0])}; {_n2(x[1])}]",
        )
        for (i, j), name, src, dest in (
            ((0, 0), "w11", "x1", "H1"), ((0, 1), "w12", "x2", "H1"),
            ((1, 0), "w21", "x1", "H2"), ((1, 1), "w22", "x2", "H2"),
        ):
            reveal = np.asarray(board.state["rv_w1"]).copy()
            reveal[i, j] = 1.0
            spot = _ZERO_22.copy()
            spot[i, j] = 1.0
            board.snap(
        ...
        for j, (name, src) in enumerate((("w_H1O", "H1"), ("w_H2O", "H2"))):
            ...
            board.snap(
                ...
                work_text=f"{name} = {_n2(w2[0, j])}   =   seta {src} -> O\nW2 é 1x2: 1 linha (a saída) por 2 colunas (os ocultos).",
            )


    def _phase_forward(self, board: _Board, x, w1, w2, z1, y1, z2, y2) -> None:
        """Phase 2: the forward pass, one arithmetic operation per frame."""
        ...
        def forward_neuron_steps(
            ...
            for k in range(2):
                ...
                spot_in[k] = 1.0
                term_lines.append(
                    f"     {'=' if k == 0 else '+'} ({_n2(weights[k])})·({_n2(inputs[k])}) = {_n2(term)}"
                )
            ...
            board.snap(
                ...
                **{("rv_y1" if focus == "l1" else "rv_y2"): reveal_y},
                ...
                hl_y1=(_cell(1.0, 0.0) if (focus == "l1" and row == 0) else
                       _cell(0.0, 1.0) if focus == "l1" else _ZERO_2.copy()),
                hl_out=1.0 if focus == "l2" else 0.0,
                rv_accum=0.0,
                work_text=f"y_{neuron} = σ({_n4(z_value)}) = {y_value:.4f}\nσ age célula por célula -- não é multiplicação de matriz.",
            )

        forward_neuron_steps(
            "H1", 0, w1[0], x, ("x1", "x2"), float(z1[0]), float(y1[0]),
            "l1", _cell(1.0, 0.0), _cell(1.0, 0.0), "hl_x",
        )
        forward_neuron_steps(
            "H2", 1, w1[1], x, ("x1", "x2"), float(z1[1]), float(y1[1]),
            "l1", np.ones(2), np.ones(2), "hl_x",
        )
        board.state["board_title"] = "Camada 2:  z = W2 · y"
        forward_neuron_steps(
    ...
    def _phase_error(self, board: _Board, y2, target, loss, diff) -> None:
        """Phase 3: the error, and why it is squared and halved."""
        # ================ fase 3: o erro ==================================
        board.snap(
            "O alvo",
    ...
    def _phase_backward(self, board: _Board, x, w2, y1, y2, sp1, sp2, gy2, gz2, gw2, gy1, gz1, gw1) -> None:
        """Phase 4: the backward pass, one arithmetic operation per frame."""
        ...
        for j, src in enumerate(("H1", "H2")):
            reveal = np.asarray(board.state["rv_gw2"]).copy()
            reveal[0, j] = 1.0
            spot = _ZERO_2.copy()
            spot[j] = 1.0
            board.snap(
        ...
        for j, dest in enumerate(("H1", "H2")):
            reveal = np.asarray(board.state["rv_gy1"]).copy()
            reveal[j] = 1.0
            spot = _ZERO_12.copy()
            spot[0, j] = 1.0
            board.snap(
        ...
        for j, dest in enumerate(("H1", "H2")):
            reveal_sp = np.asarray(board.state["rv_sp1"]).copy()
            reveal_sp[j] = 1.0
            spot = _ZERO_2.copy()
            spot[j] = 1.0
            board.snap(
            ...
            reveal_gz = np.asarray(board.state["rv_gz1"]).copy()
            reveal_gz[j] = 1.0
            board.snap(
                f"∂L/∂z_{dest}",
        ...
        for (i, j), dest, src in (
            ...
            reveal = np.asarray(board.state["rv_gw1"]).copy()
            reveal[i, j] = 1.0
            spot = _ZERO_2.copy()
            spot[j] = 1.0
            spot_z = _ZERO_2.copy()
    ...
    def _phase_chain(self, board: _Board, gw1, chain_names, chain_values, chain_partials) -> None:
        """Phase 5: the five factors of one weight's gradient, multiplied in turn."""
        ...
        board.snap(
            ...
            hl_x=_cell(1.0, 0.0), hl_w1=_cell((1.0, 0.0), (0.0, 0.0)),
            hl_w2=np.array([[1.0, 0.0]]), hl_y1=_cell(1.0, 0.0), hl_out=1.0,
