...
- [x] 58. Fundamentar a tabela de associação entre codificações e funções de perda. ✓ feito --- tabela vive apenas na Wiki (`software/nn/.wiki/Concepts/Spike-Encoding.md` "Critical Invariant", espelhada em `Concepts/Autoencoders.md`), não na tese; `CLAUDE.md` só tem a versão resumida. Ao contrário do item 57, a fundamentação aqui já era sólida: as duas citações usadas ([32] Comşa et al., "Spiking Autoencoders With Temporal Coding," Frontiers in Neuroscience 2021; [42] Manna et al., "Time series forecasting via derivative spike encoding...," Computers 2024) foram verificadas via busca na web e são reais, com autoria/veículo/tema corretos (ao contrário da citação fabricada do item 57). Reforcei a fundamentação mecanicista: adicionado parágrafo em `Spike-Encoding.md` explicando exatamente por que cada combinação errada quebra o treino, derivado diretamente do código (`SpikeCountLossImpl` soma contagem de disparos ao longo de T --- degenera para alvo 0/1 em codificação por latência; `SpikeTimeLossImpl::backward` escreve gradiente em um único índice de tempo --- cego à informação de contagem em codificação por taxa). Corrigida também uma inconsistência entre as duas tabelas (linha "Direct/contínuo": `Autoencoders.md` dizia que só `SpikeCountLoss` quebra, `Spike-Encoding.md` dizia "qualquer perda de pulso" --- alinhado para a segunda versão, mais correta) e adicionado cross-link entre as páginas.

**Seguimento (tese):** o achado inicial só cobria a Wiki --- a tese em si não mencionava a correspondência codificação↔perda. Investigado o código real do SNN-AE da Fase~00 (`E05FeatureExtraction.cpp:580`, `ae_cfg.loss_type = "mse"`) e confirmado que `SpikeCountLoss`/`SpikeTimeLoss` **não** são usadas em nenhum experimento (03/04/05) --- só em testes unitários (`trainer_gtest.cpp`, `fundamental_mechanisms_gtest.cpp`); o SNN-AE treina as três codificações (`poisson`/`latency`/`direct`) sempre com EQM, pois o alvo é sempre o sinal contínuo original, não um trem de pulsos. Adicionado parágrafo "Codificação temporal e função de perda do treinamento" em `chapters/08-proposedApproach.tex` (após a descrição das 3 codificações, §Fase 00) explicando essa distinção com precisão --- por que EQM é a escolha correta para o SNN-AE, e que a correspondência codificação-casada (`SpikeCountLoss`/`SpikeTimeLoss`) é uma capacidade geral do framework não exercitada aqui. Citações reais adicionadas a `bibliography.bib` (`comsa2021spiking`, `manna2024time`, mesmas verificadas via busca na web para a Wiki). Tese recompila limpo (112 pág., bibtex sem erros nas novas entradas).

- [x] 59. Corrigir a afirmação invertida sobre codificações temporais do SNN-AE (§08). ✓ feito --- resolve a pendência deixada no item 48. **A tese se contradizia a duas linhas de distância**: `chapters/08-proposedApproach.tex` dizia que o vencedor é a combinação de *menor* $D_{\text{verdade}}$ e, logo acima, que `poisson` ($D=1{,}93$) "supera" `direct` ($D=1{,}43$) --- os próprios números refutavam a própria regra. Os números citados vinham de uma execução preliminar (20 épocas, 2 folds), não da grade final (100 épocas, 5 folds aninhados, 3 repetições). Re-derivado contra `results/phase00/*_summary.json` (300 configs × 3 reps): a ordem real é **`direct` > `latency` > `poisson`** --- em `tiny`/EEG, `direct` ($\alpha=0{,}667$, $D=1{,}493$, rank #2 de 150) > `latency` ($\alpha=0{,}116$, $D=1{,}875$, #145) > `poisson` ($\alpha=0{,}000$, $D=1{,}997$, **#150, último**). Padrão sem exceções nos 2 sinais × 3 tamanhos; as 3 variantes `poisson` ocupam as 3 últimas posições de cada sinal. Logo a conclusão "a codificação temporal é o que torna o SNN-AE funcional" estava **invertida**: `direct` *é* a linha de base não-temporal e é a melhor ($\alpha=0{,}667 \approx$ ANN-AE de mesmo tamanho, $0{,}643$ --- retirados os pulsos, o SNN-AE vira o AE denso). Mecanismo (inferido, consistente com os dados): $\alpha$ usa a *amplitude* (mín--máx) intraclasse, logo é sensível a extremos; o ruído estocástico da codificação mapeia monotonicamente em $\alpha$ (direct: nenhum → 0,667; latency: quantização determinística → 0,116; poisson: amostragem aleatória a $T=16$ → 0,000, sobre o vértice *Falsidade*). Reescrito §08 (3 parágrafos: ordem real + mecanismo + ressalva de que $D_{\text{verdade}}$ alto não prova inutilidade para classificação, mas a Fase 00 seleciona por ele) e registrado como **resultado negativo** explícito. Removido também "linha de base degenerada" (§08, `direct` não é degenerado --- é o melhor). Wiki sincronizada (`Experiments/Experiment05.md`: tabela corrigida + nota "α≈0.5 = coin-flip" removida). Tese recompila limpo (113 pág.).
