...
Long-running commands (full pipeline, `timeout 600+`, `01_meeting01_run_loso.sh` (potentially months; `01_meeting01_run_article_profiles.sh` was deleted 2026-09-23 along with the leakage-prone `article-*.json` profiles it ran)) require explicit confirmation. Add `# CONFIRMED` to the command or set `EXPERIMENT_CONFIRMED=1`. The `expensive-experiment-guard` hook blocks unlabeled runs.
