...
Long-running commands (full pipeline ~2.5h, `timeout 600+`, `01_meeting01_run_article_profiles.sh`) require explicit confirmation. Add `# CONFIRMED` to the command or set `EXPERIMENT_CONFIRMED=1`. The `expensive-experiment-guard` hook blocks unlabeled runs.
